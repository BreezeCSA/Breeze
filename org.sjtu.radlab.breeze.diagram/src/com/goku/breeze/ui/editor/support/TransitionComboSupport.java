...
public class TransitionComboSupport extends EditingSupport {
	...
	public TransitionComboSupport(ModelEditor sme, ColumnViewer viewer, int columnIndex, BreezeNode node) {
	...
	private boolean isValidComboValue(String value) {
		if (this.input == null || this.input.length == 0) return false;
		for (Object str : this.input)
			if (value.equals(str.toString())) return true;
		return false;
	}
