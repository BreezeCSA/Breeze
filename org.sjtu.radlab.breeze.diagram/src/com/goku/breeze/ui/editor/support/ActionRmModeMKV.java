...
public class ActionRmModeMKV implements SelectionListener {
	private final BreezeNode node;
	private final TableViewer[] refreshViewer;
	private final MKVModelEditor sme;
	private final TableViewer viewer;

	public ActionRmModeMKV(BreezeNode bn, TableViewer viewer, TableViewer[] refreshViewer) {
		this.node = bn;
		this.sme = this.sme;
		this.viewer = viewer;
		...
		if (obj instanceof BreezeMode) {
			...
			for (int i = tList.size() - 1; i >= 0; --i) {
				if (tList.get(i).getSourceState().getId().equals(bm.getId())
						|| tList.get(i).getTargetState().getId().equals(bm.getId()))
					tList.remove(i);
			}
		}
		this.viewer.refresh();
