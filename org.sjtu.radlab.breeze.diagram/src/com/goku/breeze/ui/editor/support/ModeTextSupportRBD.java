...
public class ModeTextSupportRBD extends EditingSupport {
	private TextCellEditor cellEditor = null;
	private final int columnIndex;
	private final ColumnViewer[] refreshViewer;
	private final RBDModelEditor sme;
	private final TransitionComboSupportRBD[] tcs;
	private final ColumnViewer viewer;

	public ModeTextSupportRBD(RBDModelEditor sme, ColumnViewer viewer, int columnIndex, ColumnViewer[] refreshViewer,
			TransitionComboSupportRBD[] tcs) {
		super(viewer);
	...
	protected void setValue(Object element, Object value) {
		...
		case 0:
			...
			if (!value.toString().equals(bm.getId())) {
				...
				if (this.tcs != null) {
					for (TransitionComboSupportRBD tc : this.tcs)
						tc.updateInput();
				}
