 ...
 * @generated
 */
public class BreezeDiagramEditorUtil {
	 ...
	 * @generated
	 */
	private static void attachModelToResource(BreezeModel.Breeze model, Resource resource) {
	 ...
	 * @generated NOT
	 */
	public static Resource createDiagram(IProject prj, String fileName, String modelPostfix, String diagramPostfix,
		...
		IPath workspacePath = prj.getWorkspace().getRoot().getLocation().append(prj.getName()).append(fileName);

		URI diagramURI = URI.createFileURI(workspacePath.toString() + diagramPostfix);
		URI modelURI = URI.createFileURI(workspacePath.toString() + modelPostfix);

		TransactionalEditingDomain editingDomain = GMFEditingDomainFactory.INSTANCE.createEditingDomain();
		progressMonitor.beginTask(BreezeModel.diagram.part.Messages.BreezeDiagramEditorUtil_CreateDiagramProgressTask, 3);
		final Resource diagramResource = editingDomain.getResourceSet().createResource(diagramURI);
		final Resource modelResource = editingDomain.getResourceSet().createResource(modelURI);
		final String diagramName = diagramURI.lastSegment();
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(editingDomain,
				BreezeModel.diagram.part.Messages.BreezeDiagramEditorUtil_CreateDiagramCommandLabel, Collections.EMPTY_LIST) {
			@Override
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info) throws ExecutionException {
				BreezeModel.Breeze model = createInitialModel();
				attachModelToResource(model, modelResource);

				Diagram diagram = ViewService.createDiagram(model, BreezeModel.diagram.edit.parts.BreezeEditPart.MODEL_ID,
				...
				try {
					modelResource.save(BreezeModel.diagram.part.BreezeDiagramEditorUtil.getSaveOptions());
	 ...
	 * @generated
	 */
	private static BreezeModel.Breeze createInitialModel() {
		return BreezeModel.breezeFactory.eINSTANCE.createBreeze();
	}
	...
	public static boolean openDiagram(Resource diagram) throws PartInitException {
		...
		page.openEditor(new URIEditorInput(diagram.getURI()), BreezeModel.diagram.part.BreezeDiagramEditor.ID);
		return true;
	}
	...
	public static Resource openModel(Shell shell, String description, TransactionalEditingDomain editingDomain) {
		...
		try {
			...
			BreezeModel.diagram.part.BreezeDiagramEditorPlugin.getInstance().logError("Unable to load resource: " + uri, we); //$NON-NLS-1$
			MessageDialog.openError(shell,
					BreezeModel.diagram.part.Messages.BreezeDiagramEditorUtil_OpenModelResourceErrorDialogTitle, NLS.bind(
							BreezeModel.diagram.part.Messages.BreezeDiagramEditorUtil_OpenModelResourceErrorDialogMessage,
							fileName));
		}
		return resource;
	}
	...
	public static void runWizard(Shell shell, Wizard wizard, String settingsKey) {
		IDialogSettings pluginDialogSettings = BreezeModel.diagram.part.BreezeDiagramEditorPlugin.getInstance()
				.getDialogSettings();
